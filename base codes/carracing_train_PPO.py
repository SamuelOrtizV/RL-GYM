...
model = PPO("CnnPolicy", env, verbose=1, tensorboard_log=logdir, buffer_size=BUFFER_SIZE)

start_time = time.time()

for i in range(1,EPISODES):
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"sac_car_racing")
    model.save(f"{models_dir}/sac_car_racing_{i}")


print("Model trained.")
# Guardar el modelo
model.save("ppo_car_racing1")
